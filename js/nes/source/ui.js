    ...
    (function($) {
        $.fn.JSNESUI = function(roms) {
            ...
            var UI = function(nes) {
                ...
                self.romContainer = $('<div class="nes-roms"></div>').appendTo('#emulator__interface__left');
                self.romSelect = $('<select></select>').appendTo(self.romContainer);
                
                self.controls = $('<div class="nes-controls"></div>').appendTo('#emulator__interface__right');
                self.buttons = {
                    pause: $('<button value="pause" class="nes-pause fa fa-pause fa-2x" disabled="disabled" type="button"></button>').appendTo(self.controls),
                    restart: $('<button value="restart" class="nes-restart fa fa-refresh fa-2x" disabled="disabled" type="button"></button>').appendTo(self.controls),
                    sound: $('<button value="enable sound" class="nes-enablesound fa fa-volume-up fa-2x" type="button"></button>').appendTo(self.controls),
                    zoom: $('<button value="zoom in" class="nes-zoom" style="display:none;" type="button"></button>').appendTo(self.controls)
                };
                self.status = $('<span class="nes-status">Booting up...</span>').appendTo('#emulator__stats');
                $('#emulator__stats').appendTo(parent);
                ...
                self.buttons.pause.click(function() {
                    if (self.nes.isRunning) {
                        ...
                        self.buttons.pause.addClass( "fa-play" );
                        self.buttons.pause.removeClass( "fa-pause" );
                        self.buttons.pause.attr("value", "resume");
                    }
                    else {
                        self.nes.start();
                        self.buttons.pause.addClass( "fa-pause" );
                        self.buttons.pause.removeClass( "fa-play" );
                ...
                self.buttons.sound.click(function() {
                    if (self.nes.opts.emulateSound) {
                        ...
                        self.buttons.sound.addClass( "fa-volume-up" );
                        self.buttons.sound.removeClass( "fa-volume-off" );
                        self.buttons.sound.attr("value", "enable sound");
                    }
                    else {
                        self.nes.opts.emulateSound = true;
                        self.buttons.sound.addClass( "fa-volume-off" );
                        self.buttons.sound.removeClass( "fa-volume-up" );
